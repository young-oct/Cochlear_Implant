# -*- coding: utf-8 -*-
# @Time    : 2021-12-06 11:24 p.m.
# @Author  : young wang
# @FileName: gridata_fast.py
# @Software: PyCharm
...
if __name__ == '__main__':
    ...
    tri = Delaunay(a)  # Compute the triangulation
    end = time.time()

    print('Delaunay takes %f', end-start)
    # Perform the interpolation with the given values:
    start = time.time()
    interpolator = LinearNDInterpolator(tri, stack.flatten())

    end = time.time()

    print('interpolator takes %f', end-start)

    start = time.time()
    values_mesh2 = interpolator(xq, zq)
    end = time.time()
    print('new values %f', end-start)
