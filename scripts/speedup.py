...
# spec = [
#     ('points', float64[:,:]),               # a simple scalar field
#     ('values', float64[:]),          # an array field
# ]
# #
# @jitclass(spec)
class polar(object):
...
# @njit
def polar2cart(num,tri,values,xq,zq):
    for i in range(num):
        # interpolator = LinearNDInterpolator(tri, values)
        interpolator = polar(tri, values)


        values_mesh2 = interpolator(xq, zq)
    return values_mesh2
...
if __name__ == '__main__':
    ...
    xyo = cooridCO(slice)
    t1  =time.time()
    print('get polar takes %f',t1-t0)

    t0  =time.time()

    values = remap(slice).flatten()
    t1  =time.time()
    print('remap takes %f',t1-t0)
    ...
    values_mesh2 = polar2cart(num, tri, values, xq, zq)
    # for i in range(num):
    #     interpolator = LinearNDInterpolator(tri, values)
    #
    #     values_mesh2 = interpolator(xq, zq)
    t1  =time.time()
    print('remap takes %f',(t1-t0)/num)
    fig,ax = plt.subplots(1,2)
    ax[0].imshow(slice)
    ax[1].imshow(values_mesh2)
    plt.show()
