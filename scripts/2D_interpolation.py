# -*- coding: utf-8 -*-
# @Time    : 2021-12-06 8:29 a.m.
# @Author  : young wang
# @FileName: 2D_interpolation.py
# @Software: PyCharm
...
if __name__ == '__main__':
    ...
    for i in range(i_dim):
        for z in range(zdim):  # pixel coordinates conversion
            ...
            _v[i, z] = stack[i, -z] # store the pixel date temporally and flip along the colume
                                        #axis


    # x,y = np.meshgrid()
    x, y = np.mgrid[0:512, 0:330]
    temp = _iz.reshape(i_dim * zdim, 2)
    # f = interp2d(temp[:,0], temp[:,1], np.ravel(_v), kind='cubic')
    # #
    # x1 = np.arange(0,512)
    # y1 = np.arange(0,512)
    #
