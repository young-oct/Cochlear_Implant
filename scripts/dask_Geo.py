# -*- coding: utf-8 -*-
# @Time    : 2021-12-05 10:41 p.m.
# @Author  : young wang
# @FileName: dask_Geo.py
# @Software: PyCharm
...
from par_dask_griddata import dask_gd2
...
def cooridCO(test_slice):
    ...
    points = _iz.reshape(i_dim * zdim, 2)
    xx = points[:,0]
    yy = points[:,1]
    z_array = _v.flatten()

    x_target = np.arange(1,512,1)
    y_target = np.arange(1,330,1)

    v = dask_gd2(xx, yy, z_array, x_target, y_target, algorithm='cubic', n_jobs=8)
